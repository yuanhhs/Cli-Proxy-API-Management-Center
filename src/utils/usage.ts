...
export function formatUsd(value: number): string {
  ...
  const fractionDigits = absolute === 0 || absolute >= 0.01 ? 2 : absolute >= 0.001 ? 4 : 6;
...
/**
 * 计算缓存命中率：缓存命中 Token / 输入侧 Token。
 */
export function calculateCacheHitRate(usageData: unknown): number {
  const details = collectUsageDetails(usageData);
  let inputTokens = 0;
  let cachedTokens = 0;
  ...
  return calculateCacheHitRateFromTotals(cachedTokens, inputTokens);
}

export function calculateCacheHitRateFromTotals(cachedTokens: number, inputTokens: number): number {
  const normalizedInput = Number.isFinite(inputTokens) ? Math.max(inputTokens, 0) : 0;
  const normalizedCached = Number.isFinite(cachedTokens) ? Math.max(cachedTokens, 0) : 0;
  if (normalizedInput <= 0) return 0;
  return Math.min(1, normalizedCached / normalizedInput);
}

export interface TokenActivityBlockDetail {
  tokens: number;
  inputTokens: number;
  outputTokens: number;
  reasoningTokens: number;
...
export interface TokenActivityData {
  blockDetails: TokenActivityBlockDetail[];
  totalTokens: number;
  rows: number;
  cols: number;
}

export interface CostActivityBlockDetail {
  cost: number;
  inputTokens: number;
  outputTokens: number;
  cacheReadTokens: number;
...
export interface CostActivityData {
  blockDetails: CostActivityBlockDetail[];
  totalCost: number;
  rows: number;
  cols: number;
}
...
interface UsageActivityGridData {
  blockDetails: Array<{
    value: number;
    models: Array<{ model: string; value: number }>;
    entries: UsageDetail[];
    intensity: number;
    startTime: number;
    endTime: number;
...
function calculateUsageActivityGrid(
  usageDetails: UsageDetail[],
  range: UsageTimeRange,
  valueSelector: (detail: UsageDetail) => number,
  nowMs: number
): UsageActivityGridData {
  const rows = 7;
  const cols = 96;
  const blockCount = rows * cols;
  const validNow = Number.isFinite(nowMs) && nowMs > 0 ? nowMs : Date.now();
  const validTimestamps = usageDetails
    .map((detail) => detail.__timestampMs ?? parseTimestampMs(detail.timestamp))
    .filter((timestamp) => Number.isFinite(timestamp) && timestamp > 0 && timestamp <= validNow);
  const configuredWindow = range === 'all' ? 0 : USAGE_TIME_RANGE_MS[range];
  const earliestTimestamp = validTimestamps.length ? Math.min(...validTimestamps) : validNow;
  const windowMs =
    range === 'all'
      ? Math.max(validNow - earliestTimestamp, USAGE_TIME_RANGE_MS['24h'])
      : configuredWindow;
  const windowStart = validNow - windowMs;
  const blockDurationMs = windowMs / blockCount;
  const blockValues = new Array<number>(blockCount).fill(0);
  const blockModelValues = Array.from({ length: blockCount }, () => new Map<string, number>());
  const blockEntries = Array.from({ length: blockCount }, () => [] as UsageDetail[]);

  usageDetails.forEach((detail) => {
    const timestamp = detail.__timestampMs ?? parseTimestampMs(detail.timestamp);
    if (
    ...
    const blockIndex = Math.min(
      blockCount - 1,
      Math.max(0, Math.floor((timestamp - windowStart) / blockDurationMs))
    );
    const value = valueSelector(detail);
    const normalizedValue = Number.isFinite(value) ? Math.max(value, 0) : 0;
    blockValues[blockIndex] += normalizedValue;
    blockEntries[blockIndex].push(detail);
    if (normalizedValue > 0) {
      const model = detail.__modelName?.trim() || 'Unknown';
      const modelValues = blockModelValues[blockIndex];
      modelValues.set(model, (modelValues.get(model) || 0) + normalizedValue);
    }
  });

  const maxValue = blockValues.reduce((max, value) => Math.max(max, value), 0);
  const intensityDenominator = Math.log1p(maxValue);
  const blockDetails = blockValues.map((value, index) => ({
    value,
    models: Array.from(blockModelValues[index], ([model, modelValue]) => ({
      model,
      value: modelValue,
    })).sort((a, b) => b.value - a.value || a.model.localeCompare(b.model)),
    entries: blockEntries[index],
    intensity: value > 0 && intensityDenominator > 0 ? Math.log1p(value) / intensityDenominator : 0,
    startTime: windowStart + index * blockDurationMs,
    endTime: windowStart + (index + 1) * blockDurationMs,
  }));

  return {
    blockDetails,
    totalValue: blockValues.reduce((sum, value) => sum + value, 0),
    rows,
    cols,
  };
}
...
/**
 * 将当前时间范围聚合为与服务健康监测一致的 7x96 活跃度网格。
 */
export function calculateTokenActivityData(
  usageDetails: UsageDetail[],
  range: UsageTimeRange,
  nowMs: number = Date.now()
): TokenActivityData {
  const activityData = calculateUsageActivityGrid(usageDetails, range, extractTotalTokens, nowMs);

  return {
    blockDetails: activityData.blockDetails.map(({ value, entries, models, ...detail }) => {
      const tokenTypes = entries.reduce(
        (totals, entry) => {
          const input = Number(entry.tokens.input_tokens);
          const output = Number(entry.tokens.output_tokens);
          const reasoning = Number(entry.tokens.reasoning_tokens);
          const legacyCached = Math.max(
            Number(entry.tokens.cached_tokens) || 0,
            Number(entry.tokens.cache_tokens) || 0
          );
          const rawCacheRead = Number(entry.tokens.cache_read_tokens);
          const rawCacheWrite = Number(entry.tokens.cache_creation_tokens);
          const cacheRead =
            Number.isFinite(rawCacheRead) && rawCacheRead > 0
              ? Math.max(rawCacheRead, 0)
              : legacyCached;
          const cacheWrite = Number.isFinite(rawCacheWrite) ? Math.max(rawCacheWrite, 0) : 0;
          totals.inputTokens += Number.isFinite(input) ? Math.max(input, 0) : 0;
          totals.outputTokens += Number.isFinite(output) ? Math.max(output, 0) : 0;
          totals.reasoningTokens += Number.isFinite(reasoning) ? Math.max(reasoning, 0) : 0;
      ...
      return {
        ...
        tokens: value,
        ...tokenTypes,
        models: models.map(({ model, value: modelTokens }) => ({ model, tokens: modelTokens })),
      };
    }),
    totalTokens: activityData.totalValue,
    rows: activityData.rows,
    cols: activityData.cols,
  };
}

export function calculateCostActivityData(
  usageDetails: UsageDetail[],
  range: UsageTimeRange,
  modelPrices: Record<string, ModelPrice>,
  nowMs: number = Date.now()
): CostActivityData {
  const activityData = calculateUsageActivityGrid(
    usageDetails,
    range,
    (detail) => calculateCost(detail, modelPrices),
    nowMs
  );
  ...
  return {
    blockDetails: activityData.blockDetails.map(({ value, entries, models, ...detail }) => {
      const breakdown = entries.reduce<CostBreakdown>(
        (totals, entry) => {
          const entryBreakdown = calculateCostBreakdown(entry, modelPrices);
          totals.inputTokens += entryBreakdown.inputTokens;
          totals.outputTokens += entryBreakdown.outputTokens;
          totals.cacheReadTokens += entryBreakdown.cacheReadTokens;
          totals.cacheWriteTokens += entryBreakdown.cacheWriteTokens;
          totals.inputCost += entryBreakdown.inputCost;
          totals.outputCost += entryBreakdown.outputCost;
          totals.cacheReadCost += entryBreakdown.cacheReadCost;
          totals.cacheWriteCost += entryBreakdown.cacheWriteCost;
          totals.totalCost += entryBreakdown.totalCost;
          return totals;
        },
        {
          inputTokens: 0,
      ...
      return {
        ...
        ...breakdown,
        cost: value,
        models: models.map(({ model, value: modelCost }) => ({ model, cost: modelCost })),
      };
    }),
    totalCost: activityData.totalValue,
    rows: activityData.rows,
    cols: activityData.cols,
  };
}
...
export function calculateCostBreakdown(
  ...
  const rawCacheReadTokens = Number(tokens.cache_read_tokens);
  const rawCacheWriteTokens = Number(tokens.cache_creation_tokens);
  ...
  const completionTokens = Number.isFinite(rawCompletionTokens)
    ...
    : 0;
  const legacyCachedTokens = Math.max(
  ...
  const cacheReadTokens = Number.isFinite(rawCacheReadTokens)
    ? Math.max(rawCacheReadTokens, 0)
    : legacyCachedTokens;
  const cacheWriteTokens = Number.isFinite(rawCacheWriteTokens)
    ? Math.max(rawCacheWriteTokens, 0)
    : 0;
  const rawTotalTokens = Number(tokens.total_tokens);
  const totalTokens = Number.isFinite(rawTotalTokens) ? Math.max(rawTotalTokens, 0) : 0;
  const cacheIsSeparateFromInput =
    cacheReadTokens + cacheWriteTokens > 0 &&
    totalTokens >= inputTokens + completionTokens + cacheReadTokens + cacheWriteTokens;
  const promptTokens = cacheIsSeparateFromInput
    ? inputTokens
    : Math.max(inputTokens - cacheReadTokens - cacheWriteTokens, 0);

  const promptCost = (promptTokens / TOKENS_PER_PRICE_UNIT) * (Number(price.prompt) || 0);
  const cacheReadCost = (cacheReadTokens / TOKENS_PER_PRICE_UNIT) * (Number(price.cache) || 0);
  const cacheWritePrice = Number(price.cacheWrite);
  const cacheWriteCost =
    (cacheWriteTokens / TOKENS_PER_PRICE_UNIT) *
    (Number.isFinite(cacheWritePrice) ? cacheWritePrice : Number(price.prompt) || 0);
  const completionCost =
    (completionTokens / TOKENS_PER_PRICE_UNIT) * (Number(price.completion) || 0);
  const total = promptCost + cacheReadCost + cacheWriteCost + completionCost;
  return {
  ...
  try {
    ...
    Object.entries(parsed).forEach(([model, price]: [string, unknown]) => {
      ...
      normalized[model] = {
        ...
        completion,
        cache,
        cacheWrite: Number.isFinite(cacheWriteRaw) && cacheWriteRaw >= 0 ? cacheWriteRaw : prompt,
      };
    });
    return normalized;
  } catch {
...
export interface ServiceHealthBlockDetail extends StatusBlockDetail {
  models: Array<{ model: string; success: number; failure: number; rate: number }>;
}

export function calculateServiceHealthData(
  usageDetails: UsageDetail[],
  range: UsageTimeRange = '7d',
  nowMs: number = Date.now()
): ServiceHealthData {
  const ROWS = 7;
  const COLS = 96;
  ...
  const earliestTimestamp = validTimestamps.length ? Math.min(...validTimestamps) : now;
  const WINDOW_MS =
    range === 'all'
      ? Math.max(now - earliestTimestamp, USAGE_TIME_RANGE_MS['24h'])
      : USAGE_TIME_RANGE_MS[range];
  const BLOCK_DURATION_MS = WINDOW_MS / BLOCK_COUNT;
  const windowStart = now - WINDOW_MS;

  const blockStats: Array<{
    success: number;
    failure: number;
    models: Map<string, { success: number; failure: number }>;
  }> = Array.from({ length: BLOCK_COUNT }, () => ({ success: 0, failure: 0, models: new Map() }));
  ...
  usageDetails.forEach((detail) => {
    ...
    const blockIndex = Math.min(
      BLOCK_COUNT - 1,
      Math.max(0, Math.floor((timestamp - windowStart) / BLOCK_DURATION_MS))
    );

    if (blockIndex >= 0 && blockIndex < BLOCK_COUNT) {
      const model = detail.__modelName?.trim() || 'Unknown';
      const modelStats = blockStats[blockIndex].models.get(model) || { success: 0, failure: 0 };
      if (detail.failed) {
        blockStats[blockIndex].failure += 1;
      ...
      blockStats[blockIndex].models.set(model, modelStats);
    }
  });

  const blocks: StatusBlockState[] = [];
  const blockDetails: ServiceHealthBlockDetail[] = [];
  ...
  blockStats.forEach((stat, idx) => {
    ...
    blockDetails.push({
      success: stat.success,
      failure: stat.failure,
      models: Array.from(stat.models, ([model, modelStats]) => {
        const modelTotal = modelStats.success + modelStats.failure;
        return {
          model,
          ...modelStats,
          rate: modelTotal > 0 ? modelStats.success / modelTotal : -1,
        };
      }).sort((a, b) => {
        const requestDelta = b.success + b.failure - (a.success + a.failure);
        return requestDelta || a.model.localeCompare(b.model);
      }),
      rate: total > 0 ? stat.success / total : -1,
      startTime: blockStartTime,
      endTime: blockStartTime + BLOCK_DURATION_MS,

...
const formatRawSourceDisplayName = (source: string) =>
  source.startsWith('t:') ? source.slice(2) : source || '-';
...
export function buildSourceInfoMap(input: SourceInfoMapInput): SourceInfoMap {
  ...
  const registerProvider = (
    ...
    Array.from(candidates).forEach((candidate) => registerIdentity(bySource, candidate, entry));
  };
...
export function resolveSourceDisplay(
  ...
  if (authIndexKey) {
    return { displayName: authIndexKey, type: '', identityKey: `auth:${authIndexKey}` };
  }

  return { displayName: '-', type: '', identityKey: 'source:-' };
}

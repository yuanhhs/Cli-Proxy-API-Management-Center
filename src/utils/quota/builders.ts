...
import type {
  ...
  AntigravityQuotaSummaryGroupPayload,
  AntigravityQuotaSummaryModelPayload,
...
const getAntigravityWindowOrder = (bucket: AntigravityQuotaBucket): number => {
  const window = bucket.window?.toLowerCase();
  return window
    ? (ANTIGRAVITY_BUCKET_WINDOW_ORDER.get(window) ?? Number.MAX_SAFE_INTEGER)
...
export function buildAntigravityQuotaGroups(
  ...
  const modelEntries: Array<[string, AntigravityQuotaSummaryModelPayload]> =
    payload.models && !Array.isArray(payload.models)
      ? Object.entries(payload.models)
      : Array.isArray(payload.models)
        ? payload.models.map((model, index) => [
            normalizeStringValue(model.displayName ?? model.display_name) ?? `model-${index + 1}`,
            model,
          ])
        : [];
  const normalizedGroups: AntigravityQuotaSummaryGroupPayload[] = [...groups];

  if (modelEntries.length > 0) {
    normalizedGroups.push({
      displayName: 'Models',
      buckets: modelEntries.map(([modelId, model]) => {
        const quotaInfo = model.quotaInfo ?? model.quota_info ?? model;
        return {
          ...quotaInfo,
          bucketId: modelId,
          displayName: model.displayName ?? model.display_name ?? modelId,
          description: model.description ?? quotaInfo.description,
        };
      }),
    });
  }
    ...
    .map((group, groupIndex): AntigravityQuotaGroup | null => {
      ...
      const parsedBuckets = buckets
        .map((bucket, bucketIndex): AntigravityQuotaBucket | null => {
          ...
          const bucketLabel =
            normalizeStringValue(bucket.displayName ?? bucket.display_name) ?? id;
...
function parseXaiPeriodInstants(
  periodStart: string | undefined,
  periodEnd: string | undefined
): { resetAtMs: number | null; periodHours: number | null } {
  const endMs = periodEnd ? new Date(periodEnd).getTime() : Number.NaN;
  const startMs = periodStart ? new Date(periodStart).getTime() : Number.NaN;
  const resetAtMs = Number.isNaN(endMs) ? null : endMs;
  const periodHours =
    resetAtMs !== null && !Number.isNaN(startMs) && resetAtMs > startMs
...
export function buildXaiBillingSummary(
  ...
  Object.assign(summary, parseXaiPeriodInstants(summary.periodStart, summary.periodEnd));
  return summary;
}
...
export function mergeXaiBillingSummaries(
  ...
  return {
    ...
    periodStart,
    periodEnd,
    ...parseXaiPeriodInstants(periodStart, periodEnd),

...
export function useAuthFilesOauth(options: UseAuthFilesOauthOptions): UseAuthFilesOauthResult {
  ...
  const requireExcludedReady = useCallback(() => {
    if (excludedReadyRef.current) return true;
    showNotification(t('notification.refresh_failed'), 'error');
    return false;
  }, [showNotification, t]);

  const requireModelAliasReady = useCallback(() => {
    if (modelAliasReadyRef.current) return true;
    showNotification(t('notification.refresh_failed'), 'error');
    return false;
  }, [showNotification, t]);
  ...
  const deleteExcluded = useCallback(
    (provider: string) => {
      ...
      showConfirmation({
        ...
        onConfirm: async () => {
          if (!requireExcludedReady()) return;
    ...
    [loadExcluded, requireExcludedReady, showConfirmation, showNotification, t]
  );
  ...
  const deleteModelAlias = useCallback(
    (provider: string) => {
      showConfirmation({
        ...
        confirmText: t('common.confirm'),
        onConfirm: async () => {
          if (!requireModelAliasReady()) return;
          try {
    ...
    [loadModelAlias, requireModelAliasReady, showConfirmation, showNotification, t]
  );
  ...
  const handleMappingUpdate = useCallback(
    ...
    [loadModelAlias, modelAlias, requireModelAliasReady, showNotification, t]
  );

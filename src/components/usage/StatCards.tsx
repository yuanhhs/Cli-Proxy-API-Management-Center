...
export function StatCards({ usage, loading, modelPrices, nowMs, sparklines }: StatCardsProps) {
  ...
  const { tokenBreakdown, cacheInputTokens, rateStats, totalCost, latencyStats } = useMemo(() => {
    const empty = {
  ...
  const cacheHitRate = calculateCacheHitRateFromTotals(
  ...
  const statsCards: StatCardData[] = [
    ...
    {
      key: 'cache-rate',
      label: t('usage_stats.overall_cache_hit_rate'),
      icon: <IconChartLine size={16} />,
      accent: '#0ea5a4',
      accentSoft: 'rgba(14, 165, 164, 0.18)',
      accentBorder: 'rgba(14, 165, 164, 0.34)',
      value: loading ? '-' : `${(cacheHitRate * 100).toFixed(1)}%`,
      meta: (
        <>
          <span className={styles.statMetaItem}>
            {t('usage_stats.cached_tokens')}:{' '}
          ...
          <span className={styles.statMetaItem}>
            {t('usage_stats.input_tokens')}: {loading ? '-' : formatCompactNumber(cacheInputTokens)}
          </span>
        </>
      ),

...
export function useSparklines({
  ...
  const lastHourSeries = useMemo(() => {
    if (!usage) return { labels: [], requests: [], tokens: [], cacheRates: [] };
    if (!Number.isFinite(nowMs) || nowMs <= 0) {
      return { labels: [], requests: [], tokens: [], cacheRates: [] };
    }
    const details = collectUsageDetails(usage);
    if (!details.length) return { labels: [], requests: [], tokens: [], cacheRates: [] };
    ...
    details.forEach((detail) => {
      ...
      inputTokenBuckets[minuteIndex] +=
        typeof tokens.input_tokens === 'number' ? Math.max(tokens.input_tokens, 0) : 0;
      cachedTokenBuckets[minuteIndex] += Math.max(
    ...
    const cacheRates = inputTokenBuckets.map(
      (inputTokens, index) =>
        calculateCacheHitRateFromTotals(cachedTokenBuckets[index], inputTokens) * 100
    );

    return { labels, requests: requestBuckets, tokens: tokenBuckets, cacheRates };
  }, [nowMs, usage]);
  ...
  const cacheRateSparkline = useMemo(
    () =>
      buildSparkline(
        { labels: lastHourSeries.labels, data: lastHourSeries.cacheRates },
        '#0ea5a4',
        'rgba(14, 165, 164, 0.18)'
      ),
    [buildSparkline, lastHourSeries.cacheRates, lastHourSeries.labels]
  );
